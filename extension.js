...
class BalanceIndicator extends PanelMenu.Button {
    ...
    _boot() {
        ...
        if (this._s.get_string('snapshot-date') !== todayStr()) {
            // 判断旧快照是否是昨天：若是，说明跨天了，应补发 Webhook
            const yesterday = new Date();
            yesterday.setDate(yesterday.getDate() - 1);
            const yesterdayStr = `${yesterday.getFullYear()}-${String(yesterday.getMonth()+1).padStart(2,'0')}-${String(yesterday.getDate()).padStart(2,'0')}`;
            const snapDate = this._s.get_string('snapshot-date');
            const shouldWebhook = (snapDate === yesterdayStr);
            dbg(`今天无快照，执行 _doSnapshot，补发Webhook=${shouldWebhook}（快照日期=${snapDate}，昨天=${yesterdayStr}）`);
            this._doSnapshot(shouldWebhook, () => {
    ...
    _doSnapshot(sendWebhook, done) {
        ...
        Promise.allSettled([p1, p2]).then(([r1, r2]) => {
            ...
            // ★ 在写入新的 yesterday-consumed 之前，先读取"前天"的消耗值（用于 Webhook 对比）
            ...
            if (sendWebhook) this._sendWebhook(r1, r2, cfg, yest1, yest2, prevY1, prevY2);
            this._render(r1, r2, cfg);
            if (done) done();
        });
    }
    ...
    // ── Discord Webhook ───────────────────────────────────────────────────────
    _sendWebhook(r1, r2, cfg, yest1, yest2, prevY1, prevY2) {
        ...
        const mkField = (result, name, yest, prevYest) => {
            if (result.status !== 'fulfilled') return null;
            let diffLine = '';
            if (yest >= 0 && prevYest >= 0) {
                const d = yest - prevYest;
                if (Math.abs(d) < 0.00001) diffLine = '↔️ 与前日持平';
                else if (d > 0)            diffLine = `📈 较前日增加：+$${d.toFixed(4)}`;
                else                       diffLine = `📉 较前日减少：-$${Math.abs(d).toFixed(4)}`;
            }
            const lines = [
                `📅 昨日消耗：**$${yest >= 0 ? yest.toFixed(4) : '?'}**`,
            ];
            if (diffLine) lines.push(diffLine);
            lines.push(`📊 总用量：**$${result.value.toFixed(4)}**`);
            return {
                name: `🌐 ${name}`,
                value: lines.join('\n'),
                inline: true,
        ...
        const fields = [
            mkField(r1, cfg.name1, yest1, prevY1),
            mkField(r2, cfg.name2, yest2, prevY2),
        ].filter(Boolean);
